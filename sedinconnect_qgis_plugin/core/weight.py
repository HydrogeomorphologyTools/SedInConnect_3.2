...
class WeightCalculator:
    ...
    def compute(self, dtm_path: Path, window_size: int,
                ...
                normalize: bool = False, sink_flag: int = 0,
                n_workers: int = None, chunk_size: int = 1024) -> Path:
        """Compute Cavalli weighting factor with optimized parallel processing"""
        start_time = time.time()
        chunk_size = int(chunk_size) if chunk_size and chunk_size > 0 else 1024
        ...
        # Write results and compute true global maximum from core pixels
        ...
        for ri_chunk, pos in zip(results, chunk_positions):
            ...
            valid_core = core_chunk[~np.isnan(core_chunk)]
            if len(valid_core) > 0:
                core_max = float(valid_core.max())
                if core_max > max_ri_global:
                    max_ri_global = core_max
