...
class ProcessingParams:
    ...
    d_up_path: Optional[Path] = None
    d_down_path: Optional[Path] = None
    ...
    def from_dict(cls, data: dict) -> 'ProcessingParams':
        ...
        path_fields = ['dtm_path', 'output_path', 'weight_path', 'target_path',
                       'sink_path', 'original_dtm_path', 'roughness_path',
                       'weight_output_path', 'd_up_path', 'd_down_path']
        for field in path_fields:
            if field in data and data[field] is not None and data[field] != '':
                data[field] = Path(data[field])
            elif field in data:
                data[field] = None
        # Defaults for optional boolean flags and parameters if missing
        if 'fill_dtm' not in data:
            data['fill_dtm'] = False
        if 'show_preview' not in data:
            data['show_preview'] = True
        if 'n_workers' not in data:
            data['n_workers'] = None
