...
from sedinconnect.core.native.areadinf import accumulate_dinf, accumulate_dinf_multi
...
class ConnectivityProcessor:
    ...
    def process(self, params: ProcessingParams):
        ...
        if params.fill_dtm:
            ...
            filled_path = params.dtm_path.parent / (
                params.dtm_path.stem + "_filled.tif")
    ...
    def compute_connectivity_outlet(self, dtm_path: Path, cell_size: float,
        ...
        if sink_flag == 0:
            ...
            self.log("Computing D8 flow direction (native)...")
            tif_fdir8_ar, tif_sd8_ar = compute_d8_flowdir(
                tif_ar, cell_size=cell_size, nodata=ndv_val,
                log_func=self.log)

            p_file = filename[:-4] + "p.tif"
            sd8_file = filename[:-4] + "sd8.tif"
            save_raster(tif_fdir8_ar, Path(p_file), geotransform, projection)
            save_raster(tif_sd8_ar, Path(sd8_file), geotransform, projection)
            ...
            save_raster(tif_dirinf_ar, Path(ang_file), geotransform, projection,
                        nodata=-1.0)
        ...
        # Flow direction for propagation - any non-valid direction (outside [1..8]) is an outlet (np.nan)
        tif_fdir8_arr_use = np.full_like(tif_fdir8_ar, np.nan)
        mask_valid = (tif_fdir8_ar >= 1) & (tif_fdir8_ar <= 8)
        tif_fdir8_arr_use[mask_valid] = tif_fdir8_ar[mask_valid]

        # Clamp slope to [0.005, 1]
        tif_sd8_ar_clamped = tif_sd8_ar.copy()
        tif_sd8_ar_clamped[(tif_sd8_ar_clamped >= 0) &
                           (tif_sd8_ar_clamped < 0.005)] = 0.005
        tif_sd8_ar_clamped[tif_sd8_ar_clamped > 1] = 1

        s_file = filename[:-4] + "s.tif"
        save_raster(tif_sd8_ar_clamped, Path(s_file), geotransform, projection)
        ...
        D_down_ar = compute_weighted_flow_length(
            tif_fdir8_arr_use, Ws_1, cell_size, self.log)
        D_down_ar[D_down_ar == 0] = 1

        # Native AreaDinf flow accumulation - single pass multi-tensor
        self.log("Computing D-infinity flow accumulation (native unified pass)...")
        tif_sca_ar, acc_W_ar, acc_S_ar = accumulate_dinf_multi(
            tif_dirinf_ar, cell_size=cell_size,
        ...
        save_raster(tif_sca_ar, Path(sca_file), geotransform, projection)
        save_raster(acc_W_ar, Path(accW_file), geotransform, projection)
        save_raster(acc_S_ar, Path(accS_file), geotransform, projection)

        # Compute IC components
        ...
        # Cleanup
        temp_files = [p_file, sd8_file, ang_file, sca_file,
                      accW_file, accS_file, s_file]
        for f in temp_files:
            if Path(f).exists():
                try:
                    os.remove(f)
        ...
        if sink_flag == 1:
            for f in ["sinked_dtm.tif", "sinked_fdir8.tif",
    ...
    def compute_connectivity_targets(self, dtm_path: Path, cell_size: float,
        ...
        if sink_flag == 0:
            ...
            save_raster(tif_fdir8_ar, Path(p_file_orig),
                        geotransform, projection)
            save_raster(tif_sd8_ar, Path(sd8_file_orig),
                        geotransform, projection)

            # --- native Dinf ---
            self.log("Computing D-infinity flow direction (native)...")
            tif_fdirinf_ar, _ = compute_dinf_flowdir(
                tif_ar, cell_size=cell_size, nodata=ndv_val,
                log_func=self.log)

            angt_file = filename[:-4] + "angt.tif"
            save_raster(tif_fdirinf_ar, Path(angt_file),
                        geotransform, projection, nodata=-1.0)
            ...
            with LargeFileRasterReader(Path(sd8_file_orig)) as sd8_reader:
                tif_sd8_ar = sd8_reader.read_array()
            with LargeFileRasterReader(Path(angt_file)) as ang_reader:
                tif_fdirinf_ar = ang_reader.read_array()

        # Mask targets (block flow through target cells)
        ...
        tif_fdir8_ar_taudem[tif_fdir8_ar_taudem == -1.0] = -9999
        save_raster(tif_fdir8_ar_taudem, Path(p_file),
                    geotransform, projection, -9999)

        ang_file = filename[:-4] + "ang.tif"
        tif_fdirinf_ar_masked = tif_fdirinf_ar.copy()
        tif_fdirinf_ar_masked[target_ar == 10] = -1000
        tif_fdirinf_ar_masked[tif_fdirinf_ar_masked == -9999.0] = -9999
        save_raster(tif_fdirinf_ar_masked, Path(ang_file),
                    geotransform, projection, -9999)
        ...
        tif_sd8_ar_clamped[(tif_sd8_ar_clamped >= 0) &
                           (tif_sd8_ar_clamped < 0.005)] = 0.005
        tif_sd8_ar_clamped[tif_sd8_ar_clamped > 1] = 1
        tif_sd8_ar_clamped[tif_sd8_ar_clamped < 0] = -1
        s_file = filename[:-4] + "s.tif"
        save_raster(tif_sd8_ar_clamped, Path(s_file), geotransform, projection)
        ...
        tif_fdir8_for_prop = np.full_like(tif_fdir8_ar_prop, -9999.0)
        mask_valid = (tif_fdir8_ar_prop >= 1) & (tif_fdir8_ar_prop <= 8)
        tif_fdir8_for_prop[mask_valid] = tif_fdir8_ar_prop[mask_valid]
        tif_fdir8_for_prop[tif_fdir8_ar_prop == -1000] = np.nan

        D_down_ar = compute_weighted_flow_length(
            tif_fdir8_for_prop, Ws_1, cell_size, self.log)
        D_down_ar[D_down_ar == 0] = 1
        D_down_ar[D_down_ar < 0] = np.nan
        D_down_ar[target_ar == 10] = np.nan

        # AreaDinf flow accumulation on masked angle grid - single pass multi-tensor
        self.log("Computing upslope component (AreaDinf native unified pass)...")
        tif_sca_ar, acc_W_ar, acc_S_ar = accumulate_dinf_multi(
            tif_fdirinf_ar_masked, cell_size=cell_size,
            weight=tif_wgt_ar, slope=tif_sd8_ar_clamped,
            nodata_ang=-1.0, nodata_weight=-9999.0, log_func=self.log
        )
        ...
        temp_files = [
            p_file, s_file, ang_file, angt_file,
            sca_file, accW_file, accS_file,
            str(target_raster),
            filename[:-4] + "ad8.tif",
            filename[:-4] + "p.tif",
            filename[:-4] + "sd8.tif",
